...
import tables
from config import test_set_portion, dataset_name, lr_init, first_layer_filters, batch_size, use_cpu, training_epochs, kernel_size, use_sample_weights
...
with tables.open_file(os.path.join('data',dataset_name+'.h5'), 'r') as dataset:
    ...
    models_to_delete = [os.path.join(model_path,f) for f in os.listdir(model_path)[:-1]]
    [os.remove(mod) for mod in iter(models_to_delete)]
    model = load_model(os.path.join(model_path, os.listdir(model_path)[0]))
