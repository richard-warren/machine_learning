...
def get_frame(file, percentiles=(0, 100)):
    ...
    img_np = 1 - np.clip((img_np-pix_min) / (pix_max-pix_min), 0, 1)  # scale to between 0 and 1
    return img_np
...
def preview_vid(folder, frames_to_show=100, fps=30, close_when_done=False):
    ...
    # initialize window
    im_plot = plt.imshow(np.zeros((200, 256), dtype='float32'), cmap='plasma', vmin=0, vmax=1)
    plt.show()
